...
class SplitInDayAndHour
    ...
    start_date.step(end_date, HOUR_STEP).each do |data|
      if day == data.to_date #=> sono nello stesso giorno, uso to_date perchè altrimenti mi vedeva anche le ore
        if end_date == data #=> sono arrivato alla fine inserisco il doc e esco
          unless hours.empty?
            days << make_day(day, hours, event_status, unaviable_capacity)
          end
          break
        end
        hour = data.hour + 1
        ...
        unless days << make_day(day, hours, event_status, unaviable_capacity)
        end
        day += 1
        hour = data.hour + 1
        hours = [make_hour(remit, data)]
  ...
  def self.make_day(day, hours, event_status, unaviable_capacity)
    ...
    {data_day: day, data_string: day.strftime('%Y%m%d'), remit: unaviable_capacity, is_last: is_last.to_i, hours: hours}
  end
